...
* `newNodeType` - Boolean value that controls whether the button for creation of new node types should be displayed or not.
  The default value is `true`.
...
* `dynamic` (optional) - determines whether the interface should be dynamic, which means that the number of interfaces created of this definition can be changed in the editor.
  ...
  The format is `{name} {direction}`.
  The property also has the optional attribute `dynamicCounter` set to `true`.
...
* `bus` (optional) - interfaces can be changed to represent a bus:
    * `bus.type` - sets the interface to behave like a bus. Can be either set to `"oneSided"` or `"twoSided"` resulting in bigger or smaller variation of the bus.
    * `bus.size` - sets the size of the bus in pixels.
